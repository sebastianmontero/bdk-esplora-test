...
async fn main() {
    ...
    let mut wallet = bdk::Wallet::new_no_persist(
        descriptor,
        None,
        bdk::bitcoin::Network::Testnet,
    ).unwrap();
    let blockchain = esplora_client::Builder::new(endpoint).build_async().unwrap();

    let prev_tip = wallet.latest_checkpoint();
    let keychain_spks = wallet
        .spks_of_all_keychains();
    let (update_graph, last_active_indices) = blockchain
    .update_tx_graph(keychain_spks, None, None, request_stop_gap, parallel_requests)
    .await.unwrap();
    let missing_heights = wallet.tx_graph().missing_heights(wallet.local_chain());
    let chain_update = blockchain.update_local_chain(prev_tip, missing_heights).await.unwrap();
    ...
    let txid = Txid::from_str("c986542760cce19005b436fc45675a43819084bf37f683dae06e4816e77e8e9f").unwrap();
    ...
    let confirmation_time: bdk::chain::ConfirmationTime = canonical_tx.chain_position.cloned().into();
    match confirmation_time {
        ...
        bdk::chain::ConfirmationTime::Unconfirmed { last_seen } => { dbg!(last_seen); }
    }
}
